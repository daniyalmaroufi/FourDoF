#!/usr/bin/env python3
"""ROS1 node wrapping TranslationalController: exposes ITT/OTT over topics/services.

Params:
  ~config_file: path to a YAML file (see config/translational_config.yaml)
"""
...
from fourdof.joint_math import TranslationalJointConfig
...
from fourdof.translational_controller import TranslationalController


def _load_joints(joints_cfg: dict) -> dict:
    joints = {}
    for name, cfg in joints_cfg.items():
        min_mm = cfg.get('min_mm')
        max_mm = cfg.get('max_mm')
        if min_mm is None or max_mm is None:
            raise RuntimeError(
                f"Joint '{name}' is missing min_mm/max_mm. Set both to this joint's real "
                'travel limits in the config file before running.'
            )
        joints[name] = TranslationalJointConfig(
            motor_id=cfg['motor_id'],
            pitch_mm=cfg.get('pitch_mm', 2.54),
            min_mm=min_mm,
            max_mm=max_mm,
            default_speed_mm_s=cfg.get('default_speed_mm_s', 5.0),
            default_accel_mm_s2=cfg.get('default_accel_mm_s2'),
        )
    return joints


class TranslationalControllerNode:

    def __init__(self):
        rospy.init_node('translational_controller_node')
        ...
        device_name = config.get('device_name', '/dev/ttyUSB1')
        ...
        rospy.loginfo(f"Translational Controller connecting to {device_name} @ {baud_rate} baud "
                      f"(joints: {list(joints)})")
        self.controller = TranslationalController(device_name, baud_rate, joints)
        ...
        rospy.loginfo('translational_controller_node ready.')
        ...
        while not rospy.is_shutdown():
            for joint_name in self.controller.joint_names:
                try:
                    ...
                    msg.position = state['position_mm']
                    msg.velocity = state['velocity_mm_s']
...
def main():
    try:
        node = TranslationalControllerNode()
        node.spin()
